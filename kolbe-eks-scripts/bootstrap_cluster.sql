...
kubectl --context "${contexts[0]}" -n test-us-east-1 exec test-us-east-1-pd-0 -- /pd-ctl config placement-rules rule-group set cluster_rule 100 true
kubectl --context "${contexts[0]}" -n test-us-east-1 exec test-us-east-1-pd-0 -- /pd-ctl config placement-rules rule-bundle set --in="cluster_rule_group.json"
